...
/*
    ...
    The injection phase:
        ...
        A module should have an inject method which accepts the 
            dependencies for the module as the first argument. The inject 
            method also get's an optional done callback as the second argument. 
            This is used to tell the core that the module is done doing any 
            asynchronous startup logic.
    ...
    Modules expose a public interface, which is a proxy of the interface
        that the module defines. This proxy is just a thin wrapper that 
        indireclty invokes the interfaces methods. This proxy is used to 
        enable hot code reload without having to get rid of references to 
        dead objects.
...
module.exports = {
    ...
    */
    use: function use(name, module) {
        ...
        pd.bindAll(interface)
        pd.bindAll(module)
        pd.extend(module, interface)
